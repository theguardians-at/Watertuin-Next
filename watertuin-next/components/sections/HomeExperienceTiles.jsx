...
export default function HomeExperienceTiles({ expItems }) {
  return (
      ...
      <div className="experience-tiles">
        {(expItems || []).map((item, i) => (
            <div className="exp-tile" key={i}>
              <img alt="Sushi" loading="lazy" src={item.img}/>
              <div className="exp-tile__overlay"></div>
              <div className="exp-tile__content">
                <h3 className="exp-tile__title" dangerouslySetInnerHTML={{ __html: item.title }}></h3>
                <p className="exp-tile__sub" dangerouslySetInnerHTML={{ __html: item.sub }}></p>
              </div>
              <a className="exp-tile__arrow" href="/06_sortiment">
