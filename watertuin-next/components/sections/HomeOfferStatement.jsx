...
export default function HomeOfferStatement({ pillItems, headline, lead, cta }) {
  return (
      ...
      <section className="offer-statement">
        <div className="offer-statement__inner">
          <div className="offer-statement__left">
            <h2 className="offer-statement__headline" data-cms="homepage.offer_headline" dangerouslySetInnerHTML={{ __html: headline }}></h2>
          </div>
          <div className="offer-statement__right">
            <p className="offer-statement__lead" dangerouslySetInnerHTML={{ __html: lead }}></p>
            <div className="offer-statement__pills">
              {(pillItems || []).map((item, i) => (
                  <span className="pill highlight" dangerouslySetInnerHTML={{ __html: item.pill }}></span>
              ))}
            </div>
            <a className="offer-statement__cta" href="/05_preise" dangerouslySetInnerHTML={{ __html: cta }}></a>
          </div>
        </div>
      </section>
    </div>
